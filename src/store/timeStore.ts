...
import { TimeItem, Group } from "@/types/timeItem";

interface TimeState {
  items: TimeItem[];
  groups: Group[];
  addItem: (item: TimeItem) => void;
  updateItem: (id: string, updatedItem: Partial<TimeItem>) => void;
...
export const useTimeStore = create<TimeState>((set, get) => ({
  ...
  updateItem: (id, update) => {
    const updated = get().items.map((item) => (item.id === id ? { ...item, ...update } as TimeItem : item));
    saveToStorage({ ...get(), items: updated });
    set({ items: updated });
  },
  ...
  updateGroup: (id, update) => {
    const updated = get().groups.map((group) => (group.id === id ? { ...group, ...update } : group));
    saveToStorage({ ...get(), groups: updated });
    set({ groups: updated });
  },
...
function saveToStorage(state: Pick<TimeState, 'items' | 'groups'>) {
  localStorage.setItem('timeframe_store', JSON.stringify({
    items: state.items,
    groups: state.groups,
  }))
}